...

{
    ...
    if ($check_staff_result->num_rows > 0) {
        ...
        $check_duplicate_sql = $conn->prepare("SELECT * FROM `abroad` WHERE `staff_id` = ?");
        ...
        if ($check_duplicate_result->num_rows == 0) {
            ...
            $insert_sql = $conn->prepare("INSERT INTO `abroad` 
                (`staff_id`, `staff_name`, `faculty`, `organisation`, `country`, `type`, `sponsor`, `start_date`, `end_date`, `no_days`, `link_evidence`, `remarks`) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

            $insert_sql->bind_param(
                'ssssssssssss',
                $staff_id, $staff_name, $faculty, $organisation, $country,
                $type, $sponsor, $start_date, $end_date, $no_days,
                $link_evidence, $remarks
            );

            if ($insert_sql->execute()) {
                echo "<script>alert('New record successfully added');</script>";
                echo "<script>document.location='Abroad.php';</script>";
            } else {
...
<head>
    <meta charset="utf-8">
    <title>Add New Staff Abroad</title>
...
<body>
    <div class="container-fluid">
        <div class="container">
            <h1 class="border-bottom text-center pb-3 mb-4">Add Staff Abroad</h1>
            <form action="" method="post">
                <div class="row">
                    ...
                    <div class="col-md-12 mb-3 text-center">
                        ...
                        <a href="Abroad.php" class="btn btn-success">View Staff</a>
                    </div>
                </div>
            </form>
        </div>
