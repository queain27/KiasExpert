...

{
    ...
    if ($check_staff_result->num_rows > 0) {
        ...
        $check_duplicate_sql = $conn->prepare("SELECT * FROM `staffinternational` WHERE `staff_id` = ?");
        ...
        if ($check_duplicate_result->num_rows == 0) {
            ...
            $sql = mysqli_query($conn, "INSERT INTO `staffinternational` (`staff_id`,`staff_name`, `programme_title`,`faculty`,`link_evidence`,`remarks`) 
      VALUES ('$staff_id', '$staff_name','$programme_title','$faculty','$link_evidence','$remarks')");
      
            if ($sql) {
                echo "<script>alert('New record successfully added');</script>";
                echo "<script>document.location='Staff_International.php';</script>";
            } else {
...
<head>
  <meta charset="utf-8">
  <title>Add New Staff International</title>
